...
describe("logger", () => {
  ...
  describe("createLogSession", () => {
    const { createLogSession } = require("../lib/logger");
    ...
    test("creates log directory", async () => {
      ...
      const exists = await Bun.file(join(sessionPath, ".keep")).exists();
  ...
  describe("appendLog and readLog", () => {
    const { createLogSession, appendLog, readLog } = require("../lib/logger");
  ...
  describe("listLogSessions", () => {
    const { createLogSession, listLogSessions } = require("../lib/logger");
