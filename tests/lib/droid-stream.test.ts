...
describe("droid-stream", () => {
  ...
  describe("formatDroidReviewForFixer", () => {
    test("skips system init events", () => {
      ...
      const result = formatDroidReviewForFixer(jsonl);
    ...
    test("formats tool_call with tool name and parameters", () => {
    ...
    test("formats assistant message without label", () => {
      const jsonl = JSON.stringify({
        type: "message",
        role: "assistant",
        id: "msg",
    ...
    test("formats completion event with conclusion header", () => {
      const jsonl = JSON.stringify({
        type: "completion",
        finalText: "Review complete. Found 2 issues.",
        numTurns: 5,
        durationMs: 10000,
        session_id: "abc",
        timestamp: 123,
...
<system-reminder>
  Some system reminder text here.
    ...
    test("handles full review sequence", () => {
      ...
      // System and user events should be skipped
      ...
      expect(result).toContain("[Output]");
      expect(result).toContain("export function login()");
      expect(result).toContain("I analyzed the code.");
      expect(result).toContain("=== FINAL CONCLUSION ===");
      expect(result).toContain("No issues found.");
    });

    test("returns null for empty input", () => {
      const result = formatDroidReviewForFixer("");
